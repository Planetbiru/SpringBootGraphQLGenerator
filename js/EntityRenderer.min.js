...
`;let n=new Blob([i],{type:"text/markdown"}),s=URL.createObjectURL(n),a=document.createElement("a");a.href=s,a.download=t,a.click(),URL.revokeObjectURL(s)}formatDate(t){let e=new Date(t);return e.toLocaleDateString("en-US",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"}).replace(",","")}getSVGElement(){return this.svg}createDescription(t,e){let i=this,n=document.querySelector(e);n.innerHTML="",t.entities&&t.entities.forEach(t=>{i.addDescription(t,n)})}addDescription(t,e){let i=document.createElement("div");i.classList.add("mb-4");let n=document.createElement("input");n.type="checkbox",n.className="entity-selector",n.value=t.name,n.checked=!0;let s=document.createElement("h5");s.appendChild(n),s.appendChild(document.createTextNode(" ")),s.appendChild(document.createTextNode(t.name)),i.appendChild(s);let a=document.createElement("table");a.className="table table-bordered table-striped table-sm",a.dataset.entity=t.name,a.style.width="100%",a.style.tableLayout="fixed";let r=document.createElement("thead");r.innerHTML=`
        ...
        `,a.appendChild(r);let l=document.createElement("tbody");t.columns.forEach(t=>{let e=document.createElement("tr"),i=t.type||"";null!=t.length&&""!==t.length&&(i+=`(${t.length})`),e.innerHTML=`
                <td><input type="checkbox" class="check-column" value="${t.name||""}" checked></td>
                <td>${t.name||""}</td>
                <td>${i}</td>
                <td style="text-align: center;">${t.primaryKey?"YES":"NO"}</td>
                <td style="text-align: center;">${t.autoIncrement?"YES":"NO"}</td>
                <td style="text-align: center;">${t.nullable?"YES":"NO"}</td>
                <td>${null!=t.defaultValue?t.defaultValue:""}</td>
            `,l.appendChild(e)}),a.appendChild(l),i.appendChild(a),e.appendChild(i)}}